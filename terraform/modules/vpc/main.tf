...
# Private route table — no NAT gateway, traffic to AWS services goes via VPC endpoints
...
resource "aws_security_group" "vpc_endpoints" {
  ...
  tags = merge(local.tags, { Name = "${var.project}-${var.environment}-vpce-sg" })
}

# ─── VPC Endpoints (replaces NAT Gateway, saves ~$28/month) ──────────────────
...
resource "aws_vpc_endpoint" "ecr_api" {
  ...
  tags = merge(local.tags, { Name = "${var.project}-${var.environment}-vpce-ecr-api" })
}
...
resource "aws_vpc_endpoint" "ecr_dkr" {
  ...
  tags = merge(local.tags, { Name = "${var.project}-${var.environment}-vpce-ecr-dkr" })
}
...
resource "aws_vpc_endpoint" "secretsmanager" {
  ...
  tags = merge(local.tags, { Name = "${var.project}-${var.environment}-vpce-secretsmanager" })
}
...
resource "aws_vpc_endpoint" "logs" {
  ...
  tags = merge(local.tags, { Name = "${var.project}-${var.environment}-vpce-logs" })
}
