...
- [x] 1. Terraform bootstrap (S3 + DynamoDB state backend)
  - [x] 1.1 Create `terraform/bootstrap/` with `main.tf`, `variables.tf`, `outputs.tf`
...
- [x] 2. Root module scaffolding (`terraform/`)
  - [x] 2.1 Create `terraform/versions.tf` declaring required Terraform version (`>= 1.5`) and AWS provider (`~> 5.0`)
    - _Requirements: 9.1, 9.7_
  - [x] 2.2 Create `terraform/backend.tf` with the S3 backend block (bucket, key, region, encrypt, dynamodb_table)
    - _Requirements: 1.1, 1.2, 1.3_
  - [x] 2.3 Create `terraform/variables.tf` with all root-level variables: `aws_region`, `environment`, `project`, `vpc_cidr`, `availability_zones`, `acm_certificate_arn`, `image_tag`, `github_oauth_client_id` (sensitive), `github_oauth_client_secret` (sensitive), `backup_retention_days`, `image_retention_count`
    - Use MVP-appropriate defaults as specified in the design
    - _Requirements: 9.2, 9.3, 9.5_
  - [x] 2.4 Create `terraform/outputs.tf` declaring the four root outputs: `alb_dns_name`, `ecr_repository_url`, `rds_endpoint`, `ecs_cluster_name`
    - _Requirements: 9.6_
  - [x] 2.5 Create `terraform/terraform.tfvars.example` with placeholder values for all required variables
    - _Requirements: 9.5_

- [x] 3. VPC module (`terraform/modules/vpc/`)
  - [x] 3.1 Create `terraform/modules/vpc/variables.tf` and `terraform/modules/vpc/outputs.tf`
    - Inputs: `vpc_cidr`, `public_subnet_cidrs`, `private_subnet_cidrs`, `availability_zones`, `environment`, `project`
    - Outputs: `vpc_id`, `public_subnet_ids`, `private_subnet_ids`
    - _Requirements: 2.1, 9.1_
  - [x] 3.2 Create `terraform/modules/vpc/main.tf` implementing all VPC resources
...
- [x] 4. ECR module (`terraform/modules/ecr/`)
  - [x] 4.1 Create `terraform/modules/ecr/variables.tf` and `terraform/modules/ecr/outputs.tf`
    - Inputs: `repository_name`, `image_retention_count` (default 10), `environment`, `project`
    - Outputs: `repository_url`, `repository_arn`
    - _Requirements: 3.1, 9.1_
  - [x] 4.2 Create `terraform/modules/ecr/main.tf`
...
- [x] 5. Secrets module (`terraform/modules/secrets/`)
  - [x] 5.1 Create `terraform/modules/secrets/variables.tf` and `terraform/modules/secrets/outputs.tf`
  ...
  - [x] 5.2 Create `terraform/modules/secrets/main.tf`
...
- [x] 6. RDS module (`terraform/modules/rds/`)
  - [x] 6.1 Create `terraform/modules/rds/variables.tf` and `terraform/modules/rds/outputs.tf`
  ...
  - [x] 6.2 Create `terraform/modules/rds/main.tf`
...
- [x] 7. ALB module (`terraform/modules/alb/`)
  - [x] 7.1 Create `terraform/modules/alb/variables.tf` and `terraform/modules/alb/outputs.tf`
  ...
  - [x] 7.2 Create `terraform/modules/alb/main.tf`
...
- [x] 8. ECS module (`terraform/modules/ecs/`)
  - [x] 8.1 Create `terraform/modules/ecs/variables.tf` and `terraform/modules/ecs/outputs.tf`
  ...
  - [x] 8.2 Create IAM roles in `terraform/modules/ecs/main.tf`
  ...
  - [x] 8.3 Create ECS cluster, security group, CloudWatch log group, task definition, and service in `terraform/modules/ecs/main.tf`
...
- [x] 9. Checkpoint — ensure all modules are syntactically valid
...
- [x] 10. Root module wiring (`terraform/main.tf`)
  - [x] 10.1 Create `terraform/main.tf` calling all six modules in dependency order
...
- [x] 11. Update `backstage-portal/app-config.production.yaml`
  - [x] 11.1 Set `app.baseUrl` and `backend.baseUrl` to `${APP_BASE_URL}` (env var injected by ECS task definition)
  - [x] 11.2 Update `backend.database` connection to use `host: ${POSTGRES_HOST}`, `port: ${POSTGRES_PORT}`, `user: ${POSTGRES_USER}`, `password: ${POSTGRES_PASSWORD}` with `ssl.rejectUnauthorized: false`
  - [x] 11.3 Verify `auth.providers.github` already reads `AUTH_GITHUB_CLIENT_ID` and `AUTH_GITHUB_CLIENT_SECRET` from env vars (no change needed if already correct)
...
- [x] 13. Final checkpoint — validate everything wires together
